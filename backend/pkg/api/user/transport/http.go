...
func (h HTTP) create(c *gin.Context) {
	...
	editorId := newUser.ID.Hex()
	// editorId := c.GetString(PPA.IdKey)
	editorName := c.GetString(PPA.NameKey)

	editorOid, err := primitive.ObjectIDFromHex(editorId)
	if err != nil {
		PPA.Response(c, err)
		return
	}
	...
	//newUser := h.fromSignupRequest(&data)
	created, err := h.service.Create(c, newUser, editor)
	if err != nil {
		PPA.Response(c, err)
		return
	}
	c.JSON(http.StatusCreated, userCreated(created))
	return
}
...
func (h HTTP) viewById(c *gin.Context) {
	...
	fetchedUser, err := h.service.ViewById(c, id)
	if err != nil {
		PPA.Response(c, err)
		return
	}

	c.JSON(http.StatusOK, fetched(fetchedUser))
	return
}
...
func (h HTTP) list(c *gin.Context) {
	...
	c.JSON(http.StatusOK, fetchedAll(users))
	return
}
...
func (h HTTP) viewByEmail(c *gin.Context) {
	...
	fetchedUser, err := h.service.ViewByEmail(c, h.fromEmailRequest(&data))
	if err != nil {
		PPA.Response(c, err)
		return
	}
...
func (h HTTP) update(c *gin.Context) {
	...
	c.JSON(http.StatusOK, userUpdated(updated))
	return
}
